...
function FacultyManagement(props) {
  ...
  const callAPILoad = ({ page, rowPerPage, searchText }) => {
    ...
    const apiPromise = FactultyService.getFaculties({
      page: page,
  ...
  const callAPICreate = ({ code, name }) => {
    const apiPromise = FactultyService.createFaculty({ code, name });
  ...
  const callAPIEdit = ({ id, code, name }) => {
    const apiPromise = FactultyService.editFaculty({ id, code, name });
  ...
  const callAPIDelete = ({ id }) => {
    const apiPromise = FactultyService.deleteFaculty({ id });
    setIsShowSpinner(true);
